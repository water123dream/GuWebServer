...
Server::Server(EventLoop* loop_, int loopNum_, int port_):
                loop(loop_),
                port(port_),
                socketfd(0),
                asyncLogging(new AsyncLogging("./log/")),
...

{
    ...
    //设置日志的输出函数
    Logger::setOutputFunc(std::bind(&AsyncLogging::append, asyncLogging, std::placeholders::_1, std::placeholders::_2));
    asyncLogging->start();
}
...
void Server::newConnection(int socketfd)
{
    ...
    int connfd;
    while((connfd=::accept4(socketfd, (sockaddr *)&client_addr, &client_len, SOCK_NONBLOCK|SOCK_CLOEXEC))>0)
    {
        sockets::SetNodelay(connfd);
