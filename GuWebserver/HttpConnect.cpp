...
namespace
{
    ...
    std::string readStrFromFile(const char* path)
    {
        ...
        htmlstr[s.st_size]='\0';
        close(homefd);
        return std::string(htmlstr, s.st_size);
    }

    void strInit(string& indexstr,string& jsstr,string& cssstr,string& homestr,string& iconstr)
    {
        indexstr = readStrFromFile("static/login.html");
        jsstr = readStrFromFile("static/login.js");
        cssstr = readStrFromFile("static/login.css");
        homestr = readStrFromFile("static/home.html");
        iconstr = readStrFromFile("static/favicon.ico");
    }
}
                ...
                errnum(0)
{
    ...
    strInit(indexstr, jsstr, cssstr, homestr, iconstr);
...
void HttpConnect::http()
{
    ...
    {
        LOG_INFO<<inBuffer;//如果有请求体，就直接打印出来
        httpInfo.body = inBuffer;
...
void HttpConnect::analyseRequest()
{
    ...
    if (httpInfo.method == METHOD_GET)
    {
        if (httpInfo.path == "/hello") response += "Content-Length: 11\r\n\r\nHello World";
        else if (httpInfo.path == "/" || httpInfo.path == "login.html")
        {
            response += "Content-Type: text/html\r\nContent-Length: " + to_string(indexstr.size()) + "\r\n\r\n";
            response += indexstr;
        }else if (httpInfo.path == "/login.js")
        {
            response += "Content-Type: text/javascript\r\nContent-Length: " + to_string(jsstr.size()) + "\r\n\r\n";
            response += jsstr;
        }else if (httpInfo.path == "/login.css")
        {
            response += "Content-Type: text/css\r\nContent-Length: " + to_string(cssstr.size()) + "\r\n\r\n";
            response += cssstr;
        }//else if (httpInfo.path == "/images/login/fly.png" || )
        else if (httpInfo.path == "/home.html")
        {
            response += "Content-Type: text/html\r\nContent-Length: " + to_string(homestr.size()) + "\r\n\r\n";
            response += homestr;
        }else if (httpInfo.path == "/favicon.ico")
        {
            wrongRequest("没有此URI");
            errnum = -8;
            return;
        }
    }else if (httpInfo.method == METHOD_POST)
    {
        if (httpInfo.path == "/user/loginin")
        {
            if(mysql_real_connect(&db,"localhost","root","160210","GuUser",3306,NULL,0))  //连接数据库
            {
                LOG_INFO<<"Database connect success!";
            }
            else
            {
                LOG_ERROR<<"Database connect failed!";
            }

            if(mysql_set_character_set(&db,"utf8"))
            {
                LOG_ERROR<<"Database set_character failed!";
            }
            char query[128];
            MYSQL_RES* res = nullptr;
            MYSQL_ROW row;
            std::pair<std::string, std::string> user = sockets::UrlDecode(httpInfo.body);
            LOG_INFO<<"用户名："<<user.first<<"密码："<<user.second;
            //数据库验证....
            sprintf(query,"select password from User where username='%s'",user.first.c_str());
            ...
            if(!res->row_count) //没有查找到
            {
                ...
                response += "Content-Length: " + to_string(temp.size()) + "\r\n\r\n";
                response += temp;
            }
            else
            {
                ...
                {
                    if(!strcmp(row[0],user.second.c_str()))  //密码匹配
                    {
                        string temp("success");
                        response += "Content-Length: " + to_string(temp.size()) + "\r\n\r\n";
                        response += temp;
