...
void sockets::trim(std::string &s)
{
    if( !s.empty() )
    {
...
std::pair<std::string, std::string> sockets::UrlDecode(const std::string& str)
{
    std::string strTemp1, strUse, strPass, strTemp2;
    std::istringstream split(str);
    split>>strTemp1>>strUse>>strTemp2>>strPass;
    std::pair<std::string, std::string> res(strUse, strPass);
    return res;
}
